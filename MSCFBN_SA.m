...
img1=reshape(img,512*217,size(img,3));%原始HSI数据15
...
save('MS3DCN_SA_fe.mat','featuresTrain','featuresTest','net');
...
img_HSI=reshape(img,size(img,1)*size(img,2),size(img,3));%原始HSI数据200
img_label=reshape(gt,size(img,1)*size(img,2),1);
% Data=reshape(img,145*145,size(img,3));%原始HSI数据200
% Data=img;
% label_gt=gt;
trainpercentage=5;
% [DataTest DataTrain  TrainLabel TestLabel testindex trainindex] = samplesdivide_200(Data,label_gt, trainpercentage);
...
[allPatches,allLabels] = Create_Patches(img,gt,WS);
img1=reshape(img,512*217,size(img,3));%原始HSI数据15
% img1=reshape(Data,145*145,size(Data,2));%原始HSI数据200
%% Selecting Non-zero patches
patchesLabeled = allPatches(allLabels>0,:,:,:);    %% Total Patches of HSI
patchLabels = allLabels(allLabels>0);               %% Non Zero Labels
img1 = img1(allLabels>0,:);             %% Non Zero Labels
